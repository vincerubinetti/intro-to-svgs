...
#### CSS and the `<style>` element

Recall that SVG is defined by the same people who define web standards like [HTML](https://en.wikipedia.org/wiki/HTML) and [CSS](https://en.wikipedia.org/wiki/Cascading_Style_Sheets).
...
The special characters that precede the names are known as "[CSS selectors](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Selectors)".
There are many more advanced selectors, but you will likely only ever use the id `#` and class `.` selectors.
Without a preceding character, you are referring to the type of element itself; ie the "element name".
...
The attributes that can be applied with CSS styles are referred to as "[presentation attributes](https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/Presentation)".
In general, they are the properties that describe the "style" of elements, but not their geometry.
The attributes covered in this tutorial that can be applied with styles are:
...
SVG editing software will often prefer inline CSS styles, perhaps because they have the highest/final priority.



#### `<style>` example

Here is an example of using a combination of CSS selectors: element name, id name, and class name.
...
<i class="fas fa-lg fa-sticky-note"></i>Special note: If you want to use an online font but don't want to install it on your system, you can dynamically import the font from a url like this:
...
The `<defs>` element, like the `<style>` element, is used to define special things that aren't shown in the image but can be referenced elsewhere in the document.
...
Gradients can be used as `fill`'s or `stroke`'s instead of a solid color.
...
You can specify the start (`x1`/`y1`) and stop (`x2`/`y2`) position of the gradient in terms of % (relative to the dimensions of whatever object it is applied to).
You can add any color "stops" that you need, specifying their % through the gradient (`offset`), their color (`stop-color`), and their opacity (`stop-opacity`) if needed.
...
Markers are another thing that go in the `<defs>` element.

Markers are a way to define arrow heads or point markers on the `stroke` of `<line>`'s, `<polygons>`'s, `<polyline>`'s, or `<path>`'s.
...
For example, if they are set to `5`, and the stroke using the marker has a thickness of `3`, the resulting size (in SVG units) of the marker will be `15`x`15`.
...
This is useful for bi-directional arrows, and is likely what you will want most of the time.

Unfortunately, the `fill` and `stroke` of the marker shape itself cannot be made to automatically match the color of the stroke it is attached to; they must be set manually.
This may change in future versions of SVG.



#### <i class="fas fa-lg fa-flag-checkered"></i>  EXERCISE 6
{:.break_before}

{% include figure.html src="exercises/exercise-6.svg" class="large_img" %}
